...
const REAL_UPDATE_DATE_KEY = "fund_real_update_date_v1";
...
function calculateProfit() {
  ...
  updates.forEach(data => {
      ...
      if (!data.validCalc) {
          ...
          if (data.profitCell) { data.profitCell.textContent = formatCurrency(data.rowProfit); applyProfitColor(data.profitCell, data.rowProfit); }
      }
  });
...
function getStoredRealUpdateDate() {
try {
const raw = window.localStorage.getItem(REAL_UPDATE_DATE_KEY);
if (!raw) return null;
return raw;
} catch (e) {
return null;
...
function setStoredRealUpdateDate(value) {
try {
window.localStorage.setItem(REAL_UPDATE_DATE_KEY, value);
} catch (e) {
}
}

function shouldFetchRealPercentToday() {
const todayKey = getTodayDateKey();
const stored = getStoredRealUpdateDate();
if (stored === todayKey) {
return false;
}
const now = new Date();
...
if (!shouldFetchRealPercentToday()) {
return;
}
fetchRealPercentagesForAllFunds().then(success => {
if (success) {
const todayKey = getTodayDateKey();
setStoredRealUpdateDate(todayKey);
}
});
}
checkAndUpdate();
