...
applyNumberColor(profitCell, profit);
});
...
const totalPercent = totalAmount > 0 ? (totalProfit / totalAmount) * 100 : 0;
totalPercentElement.textContent = formatCurrency(totalPercent);
applyNumberColor(totalPercentElement, totalPercent);
}
const totalProfitElement = document.getElementById("total-profit");
totalProfitElement.textContent = formatCurrency(totalProfit);
applyNumberColor(totalProfitElement, totalProfit);
}
...
percentInput.value = "";
profitCell.textContent = "0.00";
applyNumberColor(percentInput, NaN);
applyNumberColor(profitCell, 0);
});
const totalProfitElement = document.getElementById("total-profit");
...
applyNumberColor(totalPercentElement, 0);
}
const totalAmountElement = document.getElementById("total-amount");
if (totalAmountElement) {
let totalAmount = 0;
...
function applyNumberColor(element, value) {
if (!element) {
return;
}
