...
if (percentHeader) {
percentHeader.textContent = percentSortOrder ? (percentSortOrder === "asc" ? basePercentHeaderText + " ↑" : basePercentHeaderText + " ↓") : basePercentHeaderText;
}
if (profitHeader) {
profitHeader.textContent = profitSortOrder ? (profitSortOrder === "asc" ? baseProfitHeaderText + " ↑" : baseProfitHeaderText + " ↓") : baseProfitHeaderText;
}
}
...
tryFetchFundByInput(nameInput, { showAlertOnMissing: false });
});
nameInput.addEventListener("keydown", event => {
if (event.key === "Enter") {
event.preventDefault();
tryFetchFundByInput(nameInput, { showAlertOnMissing: true });
}
});
...
{
const el = totalZfbAmountElement || document.getElementById("total-zfb-amount");
if (el) {
el.textContent = formatCurrency(totalZfbAmount);
}
}
{
const el = totalStockAmountElement || document.getElementById("total-stock-amount");
if (el) {
el.textContent = formatCurrency(totalStockAmount);
}
}
{
const el = totalAmountElement || document.getElementById("total-amount");
if (el) {
el.textContent = formatCurrency(totalHoldingAmount);
}
}
{
const el = totalPercentElement || document.getElementById("total-percent");
if (el) {
const totalPercent = totalHoldingAmount > 0 ? (totalProfit / totalHoldingAmount) * 100 : 0;
el.textContent = `${formatNumber(totalPercent)}%`;
applyProfitColor(el, totalPercent);
}
}
{
const el = totalZfbProfitElement || document.getElementById("total-zfb-profit");
if (el) {
el.textContent = formatCurrency(totalZfbProfit);
applyProfitColor(el, totalZfbProfit);
}
}
{
const el = totalStockProfitElement || document.getElementById("total-stock-profit");
if (el) {
el.textContent = formatCurrency(totalStockProfit);
applyProfitColor(el, totalStockProfit);
}
}
{
const el = totalProfitElement || document.getElementById("total-profit");
if (el) {
el.textContent = formatCurrency(totalProfit);
applyProfitColor(el, totalProfit);
}
}
if (isChromeExtensionEnv()) {
...
populateTableFromStorage();
  ...
  if (percentHeader) {
    percentHeader.addEventListener("click", () => {
      percentSortOrder = percentSortOrder === "desc" ? "asc" : "desc";
  ...
  if (profitHeader) {
    profitHeader.addEventListener("click", () => {
      profitSortOrder = profitSortOrder === "desc" ? "asc" : "desc";
